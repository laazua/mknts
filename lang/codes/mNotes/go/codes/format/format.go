...
func main() {
	...
	// 可参考 ASCII
	fmt.Println("=====输出一个值的字符(char)====")
	fmt.Printf("%c\n", 97)

	// 输出一个值的十六进制,每个字符串的字节用两个字符输出
	fmt.Println("=====输出一个值的十六进制,每个字符串的字节用两个字符输出====")
	fmt.Printf("0x%x\n", 10)
	fmt.Printf("%x\n", "abc")
	...
	// 输出带双引号的字符串
	fmt.Println("=====输出带双引号的字符串====")
	fmt.Printf("%q\n", "string")

	// Go提供了几种打印格式，用来格式化一般的Go值
	p := point{1, 2}

	fmt.Println("=====输出一个指针的值====")
	fmt.Printf("%p\n", &p)
	fmt.Println("=====输出结构体的对象值====")
	fmt.Printf("%v\n", p)
	// 如果所格式化的值是一个结构体对象，那么`%+v`的格式化输出
	fmt.Println("=====输出结构体的成员名称和值====")
	fmt.Printf("%+v\n", p)
	fmt.Println("=====输出一个值的Go语法表示方式====")
	fmt.Printf("%#v\n", p)
	fmt.Println("=====输出一个值的数据类型====")
	fmt.Printf("%T\n", p)
