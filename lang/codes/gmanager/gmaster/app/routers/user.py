...
async def add(post: CreateUser, _ = Depends(get_current_user)):
...
async def remove(name: str, _ = Depends(get_current_user)):
...
async def fix(post: UpdateUser, _ = Depends(get_current_user)):
...
async def select(_ = Depends(get_current_user)):
