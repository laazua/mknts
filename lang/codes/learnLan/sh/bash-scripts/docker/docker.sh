...
echo "
    ...
    > 容器命令
		...
		nsenter --target $(docker inspect -f {{.State.Pid}} nginx) --mount --uts --ipc --net --pid
	    docker cp containerID:path/fileName  path    将容器中的文件拷贝到宿主机上
		docker export e215b7c52846 > nginx.tar    导出容器
		docker import nginx.tar test/nginx:v1    导入容器
	...
	## docker 网络模式
	  -- host模式: 容器和宿主机共享network namespace    (配置: --network=host)
	  -- containe模式: 容器和另外的多个容器共享network namepace(k8s pod就是如此)    （配置: --network=container:ID）
	  -- none模式: 容器有独立的network namespace, 但并没有对其进行任何网络设置,如veth pair和网桥连接,配置IP等    (配置: --network=none)
	  -- bridge模式: 启动docker deamon时,宿主机会创建一个docker0的虚拟网桥,此时宿主机上启动的所有容器都会连接到这个虚拟网桥上,默认为该模式    (配置: --network=bridge)
"
