...
func  main()  {
	startTime := time.Now()
	url := "https://download.jetbrains.com/go/goland-2020.2.2.dmg"  //下载文件的地址
	downloader := NewFileDownloader(url, "", "", 10)
	if err := downloader.Run(); err != nil {
		log.Fatal(err)
	}
	fmt.Printf("\n 文件下载完成,耗时: %f second", time.Now().Sub(startTime).Seconds())
}

//head获取要下载的文件的基本信息(header)使用HTTP Method Head
func (d *FileDownloader) head() (int, error){
	...
	if resp.StatusCode > 299{
		return 0, errors.New(fmt.Sprintf("Can't process, response if %v", resp.StatusCode))
	}
