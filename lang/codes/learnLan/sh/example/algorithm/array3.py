...
# -*- coding:utf-8 -*-
"""
定义一个int型的数组,包含40个元素,用来存储每个学员的成绩,循环产生40个0-100之间的随机整数,
(1) 将它们存储在一维数组中,然后统计成绩低于平均分的学员的人数,并输出出来
(2) 将这40个成绩从高到低输出
"""

from __future__ import division    #实现精确的除法,如：4/3 = 1.333333
import random

def make_score(num: int) -> list:
    score = [random.randint(0,100) for i in range(num)]
    return score


def less_average(score: int) -> int:
    num = len(score)
    sum_score = sum(score)
