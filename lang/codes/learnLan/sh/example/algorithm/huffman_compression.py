...
# -*- coding:utf-8 -*-
"""
user serveral ways to compress string `everyday is awesome
1. user simple bits to replace ASCII value
...
class HuffmanCompressoin:
    ...
    def __init__(self, string):
        ...
        for char, cnt in counter.items():
            heapq.heappush(heap, HuffmanCompressoin.Trie(cnt, char))

        while len(heap) != 1:
            left = heapq.heappop(heap)
            right = heapq.heappop(heap)
            trie = HuffmanCompressoin.Trie(left.val, right.val)
