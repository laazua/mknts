    ...
    :before-close="handleClose">
      <el-form v-loading="loading">
        <el-form-item label="区服地址" label-width="auto">
          <el-input v-model="zoneForm.zones[0].ip" autocomplete="off" placeholder="开服ip"></el-input>
        </el-form-item>
        <el-form-item label="区服名称" label-width="auto">
          <el-input v-model="zoneForm.zones[0].name" autocomplete="off" placeholder="区服名称"></el-input>
        </el-form-item>
        <el-form-item label="区服ID" label-width="auto">
          <el-input v-model="zoneForm.zones[0].id" autocomplete="off" placeholder="区服ID"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    openZone() {
      this.$confirm("是否添加区服" + this.zoneForm.zones[0].name + ':' + this.zoneForm.zones[0].id, "提示", {
        confirmButtonText: "是",
        cancelButtonText: "否",
        type: "warning",
      }).then(() => {
        ...
        AddZone(this.zoneForm).then((response) => {
          if (response.code === 20000) {
            this.$message({
              message: "添加区服: " + this.zoneForm.zones[0].name + ':' + this.zoneForm.zones[0].id + " 成功!",
              type: "success",
            });
          } else {
            this.$message({
              message: "添加区服: " + this.zoneForm.zones[0].name + ':' + this.zoneForm.zones[0].id + " 失败!",
              type: "warning",
            });
          }
          this.loading = false
    ...
    manZone() {
      ...
      this.$confirm("所选区服是否执行: " + this.vtarget, "提示", {
        ...
        const data = {"zones": this.multipleSelection, "target": this.vtarget}
        this.loading = true
        // console.log(data)
