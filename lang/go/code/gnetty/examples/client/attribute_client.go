...
// 属性存储客户端示例
// 启动：go run examples/client/attribute_client.go
func attributeMain(address string) {
	...
	client.SetHandler(func(ch *channel.Channel) {
		...
		h := handler.NewSimpleInboundHandler().
			OnActive(func(ctx channel.ChannelContext) {
				fmt.Println("[Attribute Client] Connected to server")
				fmt.Println("Commands: login, info, logout, <message>")
			}).
			OnInactive(func(ctx channel.ChannelContext) {
				fmt.Println("[Attribute Client] Disconnected")
			}).
	...
	// 读取用户输入并发送
	fmt.Println("Type command and press Enter (Ctrl+C to exit)")
