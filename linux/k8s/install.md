...
# 安装cilium(如果需要支持gateway api需参考官网配置values.yaml文件)
...
- 节点角色标签
```bash
kubectl label nodes node-01 node-role.kubernetes.io/master=""
kubectl label nodes node-03 node-role.kubernetes.io/worker=""
```
...
spec:
  addresses:
  - 192.168.165.70-192.168.165.79  # 负载均衡的IP地址范围
